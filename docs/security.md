...
- [**Token theft**](#token-theft): `src/dispatch/sbx-runner.ts` builds the `sbx exec` child environment from a **positive allow-list** scoped to a few well-defined categories:
  ...
  **Operator note — proxy URLs forward verbatim.** `HTTP_PROXY` / `HTTPS_PROXY` (and the lowercase variants) are passed through unchanged. If the URL embeds credentials (`http://user:pass@proxy/`), those credentials flow into the sandbox env. This is not a regression — under the threat model we're protecting *from* the sandbox, and the agent needs the proxy URL to make outbound requests either way — but operators using cred-in-URL proxies should consider system-level auth (`.npmrc`, `~/.curlrc`, the system keychain, NTLM/Kerberos) to keep secrets out of process env.

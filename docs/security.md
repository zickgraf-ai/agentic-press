...
- [**Confused deputy**](#confused-deputy): the `POST /sessions` payload is constructed only from the manifest file passed on argv. The dispatch CLI writes `.mcp.json` into the workspace but never reads it back. No sandbox is running when the manifest is parsed.
